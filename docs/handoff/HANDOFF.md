...
| KI-13 | ~~Info~~ **Fixed** | Closing a deletion request whose account was already gone was recorded as **Declined**, with the reason "The account was already removed." In a GDPR record that says the organisation refused a deletion, which never happened. It now gets its own status, **Closed · nothing to delete**, through a close path the server allows only when the account really is gone. A migration relabels requests already closed the old way | `lib/data-deletion.ts`, `components/privacy/deletion-request-actions.tsx` | Needs a request whose user row was deleted |
